# pkgadd.conf.5.pod - documentation for pkgadd.conf(5) manual page
...
This file can be used to alter the way L<pkgadd(8)> upgrade and/or
install the package.
...
F</etc/X11> (subdirectories included), unless it is the files
...
will install it under F</var/lib/pkg/rejected/>.  The user is then
free to examine/use/remove that file manually or use L<rejmerge(8)> to
...
All lines which begin with the C<#> character are comments and will
not be parsed.  The first three rules will cause L<pkgadd(8)> to never
upgrade anything in F</var/log> (subdirectories included), never
upgrade anything in F</var/spool/cron> and never upgrade
F</var/run/utmp> file.  The last two rules will cause L<pkgadd(8)> to
upgrade any F</etc/rc*> files, except F</etc/rc.conf> system settings.
...
Configuration file for L<pkgadd(8)>.
