 ...
 *
 * This header file defines the `pkgutil` class, which provides core
 * functionalities for package management, including database
 * operations, package installation, removal, conflict detection, and
 * archive handling.
 *
 ...
 * packages. It handles database interactions, package file operations
 * (opening, installing, footprinting), conflict resolution, and
 * system integration tasks like running `ldconfig`.
 *
 * It uses a database to track installed packages and their files, and
 * interacts with libarchive for handling package archive files.
 */
class pkgutil {
public:
  //! \typedef files_t
   ...
   *
   * This structure stores information about a package, including its
   ...
   *
   * \details
   * This method writes the current state of the `packages` map back
   * to the package database file, ensuring data persistence.
   * It implements a safe commit mechanism involving a temporary file
   * and backup to minimize data corruption in case of errors.
   */
  void db_commit();
   ...
   *
   * \details
   * This method adds a new package record to the internal `packages`
   * map.  Note that this method does not directly write to the
   * database file; `db_commit()` must be called to persist changes.
   */
   ...
   * attempts to delete all files associated with the package from the
   * filesystem.  It handles file deletion carefully, avoiding
   * deletion of files that are also part of other installed packages.
   */
   ...
   *
   * \details
   * This overload of `db_rm_pkg` allows for selective file retention
   * during package removal.  Files listed in `keep_list` will not be
   * deleted even if they belong to the package being removed.
   */
   ...
   * into account a `keep_list` of files that should not be deleted.
   * It also updates the package database to remove references to the
   * deleted files from package records.
   */
   ...
   * the database and filesystem.  It identifies files that would be
   * overwritten or conflict with existing installations.
   */
   ...
   *         (`pkginfo_t`).
   * \throws std::runtime_error If the package file cannot be opened
   *                            or read, or if the package format is
   *                            invalid.
   *
   * \details
   ...
   *                            problems, or other installation
   *                            issues.
   *
   * \details
   * This method installs a package from a given package file.
   * It uses libarchive to extract files from the package archive to
   ...
   *
   * \details
   * This method runs the `ldconfig` utility (if available) to update
   * the shared library cache after package installation or removal,
   * ensuring that the system recognizes newly installed or removed
   * shared libraries.
   */
  void ldconfig() const;
  ...
  */
  ...
  /*!
   * \var LDCONFIG_CONF
   * \brief Path to the ldconfig configuration file (relative to
   *        root).
   */
